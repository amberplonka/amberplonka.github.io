...
*/
		...
		function createVisualization(tree, wrapperWidth) {
			...
			var path = vis.data([tree]).selectAll("path").data(nodeArray).enter().append("svg:path") 
					...
					.style("fill", function(d) { return (typeof d.data.quality !== "undefined") ? getColor(d.data.quality) : getColor(11-getGenerationNum(d.data.anum) + (isFemaleAncestor(d.data.anum) ? 1 : 0)); }) 
					.style("opacity", 1)
		...
		function mouseover(e) {
			...
			newbreadcrumbs.append("div").attr("class","ancestor").attr("style", function(d) {return "color:" +  getColor(d.data.quality)}).html(function(d) {return ((d.data.name ? d.data.name : getTranslation("Missing")));});
